...
class Report extends React.Component {
  ...
  renderProductStatistic = () => {
    ...
    paymentList.map((val, idx) => {
      ...
      transactions_detail.forEach((detail) => {
        ...
        let cari = successProduct.findIndex(
          (valSuccess) => valSuccess.productId == productId
        );
        if (cari == -1) {
          successProduct = [
            ...successProduct,
    ...
    productStatistic.forEach((val) => {
      ...
      let cari = successProduct.findIndex(
        (filter) => filter.productId == val.id
      );
      console.log(cari);
      if (cari !== -1) {
  ...
  render() {
    return (
      <>
        <div className="col-10 d-inline-block border" style={{height:"500px"}} >
